...
//	free(*a);
	predict_orbital_elements_t *m = (predict_orbital_elements_t*)pvPortMalloc(sizeof(predict_orbital_elements_t));
//	realloc(m, sizeof(predict_orbital_elements_t));
//	predict_orbital_elements_t *m ;
	...
	if (TWO_PI/xnodp/MINUTES_PER_DAY >= 0.15625) {
		...
		m->ephemeris_data = pvPortMalloc(sizeof(struct _sdp4));
		...
		// Allocate memory for ephemeris data
		m->ephemeris_data = pvPortMalloc(sizeof(struct _sgp4));
